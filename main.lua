	...
	if not isfile(path) then
		...
		for attempt = 1, 3 do
			local suc, result = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/skidrewrite/japan/' .. readfile('newvape/profiles/commit.txt') .. '/' .. select(1, path:gsub('newvape/', '')), true)
			end)
	...
	vape:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				...
				loadstring(game:HttpGet('https://raw.githubusercontent.com/skidrewrite/japan/'..readfile('newvape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
			]]
			...
			if not _ok then warn('[GenvPrivate] failed before teleport: ' .. tostring(_err)) end
...
if not shared.VapeIndependent then
	...
	if isfile('newvape/games/' .. gameFileId .. '.lua') then
		...
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/skidrewrite/japan/' .. readfile('newvape/profiles/commit.txt') .. '/games/' .. gameFileId .. '.lua', true)
			end)
